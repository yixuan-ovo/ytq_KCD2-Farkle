...
> **最后更新：** 2026-06-06（移动端 UX · 活泼字体 · 图鉴/选骰摘要）  
...
**发布：** `main` push 后 Cloudflare **Pages**（`kcd2-farkle`）与 **Worker**（`ytq-kcd2-farkle-api`）经控制台 Git 集成自动构建；本地改 Worker 须重启 `yarn worker:dev` 才生效。  
...
| `src/components/selection/DicePickSummary.svelte` | 选骰等待：己方/对手已选骰横向摘要（含六面概率） |
| `src/components/layout/OrnamentalDivider.svelte` | 主菜单标题与副标题间烫金装饰分隔 |
| `src/components/game/CoinFlipOverlay.svelte` | 开局金币定先后（`turn_order`）；z-index 1700（高于胜负遮罩） |
| `src/components/game/PartnerLeftOverlay.svelte` | 对手/房主离席通知卡（~3.2s 自动消失；对局中展示比分） |
...
- `rps` / `draft_rps` 完整 UI
- 粒子 / 旁白、图鉴六面改为主题色预览、骰杯倒出真 3D
- 断线宽限外的长期房间持久化（DO 冷启动仍丢局）
...
| `/room/{id}?name=昵称` | **一律进大厅**（`App.svelte`），仅预填昵称；须点加入确认。**勿**在邀请链接里带房主昵称 |
...
**加入房间：** 大厅 `JoinForm` 用 `joinRoom()`——失败（同名/满员/超时）**留在大厅**并显示错误，不跳转对局。  
**断线宽限：** WS 断开 → `awayUntil` **2 分钟**（`AWAY_GRACE_MS`）；同名且该座位无活跃 WS 可重连续局；`GameView` 监听 `visibilitychange` 自动 `tryReconnect()`。主动 `leave` 仍立即清盘。  
对局状态在 **Durable Object**；DO 冷启动或宽限过期后需重开。

### 1.4.2 多房间与换房

- **不同房间号** 可并行存在（每房间一个 DO），多组玩家互不影响。
- **同一浏览器标签** 同时只能连一个房间；换房时 `connect()` 会先 `leave` 再连新房间。
- 房主未点「离开」就另开新桌，旧房 WS 断开，对手会看到离席通知卡（非 error toast）。
...
| 类型 | 主要文件 |
|------|----------|
| 阶段提示（爆点 / 换手 / Hot Dice） | `PhaseOverlay.svelte`（**居中紧凑 toast**，`pointer-events: none`，不挡底栏；爆点仅展示 `lastBust` 未保留骰；~1.2s 自动消失） |
| 终局结算（`game_over`） | `PhaseOverlay.svelte`（**一张终局结算卡**：胜负标题 + 决胜一击 `lastTurnEnd` 骰面/分数 + 双方终局比分 + 离开/再来一局） |
| 离席通知 | `PartnerLeftOverlay.svelte`（对手主动离开或宽限到期清盘；**不用** `lastError` toast） |
| 金币定先后 | `CoinFlipOverlay.svelte`（`turn_order`；房主 `ackTurnOrder`） |
...
| 特殊骰标注 | `DiceBoard.svelte` 骰子下方 `shortName`（GSAP/CSS 掷骰动画期间隐藏，避免字留在原地） |
...
- **大厅：** `MainMenu` 参考稿布局：全宽「进入酒馆」+ 双列「骰子图鉴」「规则说明」+ 底栏「设置」；烫金外框、图标/标题/副标题**居中**；标题「亨利的骰桌」与副标题间 `OrnamentalDivider`；`JoinForm` 等弹层不变。
- **字体（`index.html` Google Fonts + `app.css`）：** 正文 `--font-sans` **Noto Sans SC**；展示标题 `--font-display` **ZCOOL KuaiLe**（快乐体）；英文装饰 `--font-serif` **Cinzel Decorative**；数字 `--font-mono` JetBrains Mono。**国内若 Google Fonts 不可达**会回退系统黑体，部署后海外/CDN 正常时可见快乐体；后续可自托管 `public/fonts/`。
- **骰子图鉴：** `DiceCollectionPanel.svelte` 读 `diceRegistry`（39 种）+ `getDieDescription`；**手机**全屏、双列紧凑 `DiceCard`、Tab 横滑、详情**底部抽屉**（点骰子展开，可收起）；**桌面**右侧 sticky 详情栏；**不**影响对局。
- **背景：** `TavernAmbience.svelte`（烛光 + 暗角 + 低对比 emoji + 六枚特殊骰隐藏面 SVG 依次高亮）；骰子层在 vignette **之上**；`absolute inset:0`。
- **对局布局（参考 PC 稿）：** 单列居中 `game-page__center`（`margin-block: auto` 于 HUD 与底栏之间）— `DiceTable` → `TurnScoreCard` 双卡：**本轮累计**（己方选中预览 `+N`；对手回合显示**对手选分**）| **当前回合累积**（已确认 `turnScore`）。对局内**无** `ScoreRulesPanel`。
- **骰盘：** 手机 3×2；**≥768px 六枚一排**；`game-page__stage` 手机顶对齐、桌面垂直居中；**已移除** `ThrowPips`。
- **选骰页：** `DiceSelector` 标题区与顶栏 `Header` 之间留 `padding-top`（`game-page__main--pick`）；确认后 `DicePickSummary` 展示己方/对手已选骰（对手须己方已确认且对手已提交，`toClientGameState` 下发）；网格对手已选虚线高亮。
- **开局规则：** 房主确认规则后 `startPending` 保持 `RulesConfigPanel`「正在开局…」，避免 WS 往返前闪回牌桌等待页。
...
- **首掷背面：** `rollCount===0 && turnScore===0` 时 `DicePiece` 显示各骰 `face-hidden`（`getPlaceholderFaceUrl`）；进入 `selecting` 后服务端 `buildDiceForPlayer` 下发 6 枚骰（特殊骰 ID + `NormalDie` 补齐）。
- **掷骰动画：** `playDiceRollAnimation`（GSAP，DOM 克隆 overlay）；`GameView` 在**己方回合**且 `prevRollCount >= 0 && rollCount` **实际递增**时设 `physicsRolling`（`prevRollCount` 初值 `-1`，进局/重连不得误触发；`selecting` 且 `rollCount===0` 时强制复位）；`DiceBoard` 测量槽位并播放（约 850ms）；动画期间活动区骰子 `hidden` + 隐藏 `shortName`；`settings.physicsEnabled` 关闭时回退 CSS `medievalRoll`（游戏内不跟随系统「减少动画」）。
...
| 主动离开（自己） | `leaveRoom()` → `intentionalLeave=true` → WS `leave` → `disconnect()`；`App` **SPA** `replaceState('/')` 回大厅（**不** `location.assign` 硬刷新）；`{#key view-roomId}` 卸载 `GameView` 防重连竞态 |
| 对手/房主离席（对方） | `leavePlayer` 清座回 `lobby`；`detectOpponentLeft` → `session.partnerLeftNotice` → `PartnerLeftOverlay` ~3.2s 自动消失；对局中附带离席前双方 `totalScore` |
| 房主离开 | 客人卡片结束后 `onLeave()` 自动回主菜单；房主若留下则继续在大厅等人 |
| 客人离开 | 房主卡片消失后留在房内大厅（可复制暗号） |
| 爆点 | `endTurn` 写入 `lastBust`；Worker 将 `phase` 置 `selecting` 并保留 `lastBust` 供 toast；掷骰时清 `lastBust`；**不**自动续掷 |
| 同名昵称 | `joinPlayer` 拒绝与房主/已有客人同名；宽限期内仅当该座位**无活跃 WS** 才允许同名重连 |
| 断线宽限 | `webSocketClose` → `markPlayerAway`（2min）；DO `alarm` 到期 `leavePlayer`；下发 `opponentAway` / `opponentAwayUntil` |
| 金币定先后 | **无特殊骰：** `startGame` → `turn_order` + `coinFlip` → `ackTurnOrder` → `selecting`。**有特殊骰：** `startGame` → `dice_selection` → 双方选完 → `turn_order` + `coinFlip` → `ackTurnOrder` → `selecting` |
...
| 切后台 / 锁屏 | WS 可能断；**2 分钟内**回前台 `tryReconnect` 续局；底栏显示对手暂离；超时 toast「连接已断开」 |
...
- [x] `PhaseOverlay.svelte`：爆点/换手居中 toast；`game_over` 终局结算卡
- [x] `PartnerLeftOverlay.svelte`：离席通知卡自动消失
- [x] `JoinForm.svelte`：input `font-size: 16px`；primary 按钮全宽
- [x] Toast / error：顶栏下 fixed toast（z-index 2100，高于胜负遮罩）
...
KCD2-Farkle/
  ...
  src/
    ...
    components/
      ...
      game/GameHud, DiceTable, DiceBoard, TurnScoreCard, PhaseOverlay, PartnerLeftOverlay, CoinFlipOverlay, …
...
- 断线：`onClose` toast；`visibilitychange` + `tryReconnect()` 在 2 分钟宽限内自动重连
...
| `rolling` | — | — | — |
| `bust`（toast 期间 `lastBust` 保留） | ✓ 轮到我可手动掷骰 | — | — |
| `turn_end` / `hot_dice` | 下一轮掷骰前 normalize | — | — |
| `game_over` | — | — | — |

爆点后 Worker 保持 `selecting` + `lastBust`；toast ~1.2s（有骰面）自动 dismiss，**须手动点掷骰**。`turn_end` 仍由下次 `roll` 前 normalize。达标收分：`bank` 直接 `game_over` 且带 `lastTurnEnd` 快照（无 `turn_end` 中间态）。
...
- [x] Cloudflare Pages + Worker **控制台 Git 集成**（push `main` 自动部署）
- [x] 控制台环境变量 `VITE_WS_BASE=wss://farkle.yixr.uno`
- [x] 域名 `farkle.yixr.uno` Pages 绑定 + Worker 路由 `/room/*`

### Phase F — 增强（后续）

- [x] 特殊骰子选骰 UI + Worker `pickDice` / `dice_selection`（`RulesConfigPanel`、`DiceSelector`、`pendingPickIds`）
- [ ] RPS、draft 选骰模式
- [x] 断线宽限 2 分钟 + 前台自动重连（`awayUntil` / `tryReconnect`）
- [ ] 房间长期持久化（DO 冷启动仍丢局）
- [ ] 动画、音效、骰子皮肤
...
- 分享链接：`shareUrl()` → `https://farkle.yixr.uno/room/ABC123`（**不含**昵称）
...
- [x] `keep` 传 `dieIds` 数组
- [x] `bank` 达到 `targetScore` → `game_over`（含 `lastTurnEnd` 决胜快照）
- [x] `start` 从 `game_over` 重开 → `turn_order`（`restartGame()` 校验房主/双方在座）
- [x] 错误包 toast（`session.lastError`）；对手离席走 `partnerLeftNotice`，**不**进 error toast
...
| 2026-06-05 | **爆点卡住**：`PhaseOverlay` toast + `pointer-events: none`；`pendingKeepIds` 响应式；爆点后 **手动**掷骰（移除自动 `roll()`） |
| 2026-06-05 | **断线宽限**：`awayUntil` 2 分钟；DO alarm 踢人；`tryReconnect` + `visibilitychange`；底栏暂离文案 |
| 2026-06-05 | **加入房间**：`joinRoom()` 失败留大厅；拒绝同名昵称；邀请链 `?name=` 一律进大厅确认 |
| 2026-06-05 | **金币定先后**：`CoinFlipOverlay` + `turn_order` / `ackTurnOrder` |
| 2026-06-05 | **部署文档**：Pages + Worker Git 自动部署为主流程；Phase E ✅ |
...
| 2026-06-06 | **终局结算卡**：达标 `game_over` 一张卡展示决胜一击（`lastTurnEnd`）+ 双方终局比分；服务端 winning `bank` 保留 `lastTurnEnd` |
| 2026-06-06 | **重开修复**：`restartSending` 不藏遮罩；错误 toast / 金币动画 z-index；`restartGame()` 显式校验并返回 boolean |
| 2026-06-06 | **语录死循环**：`useTavernQuote` `shownGameOverQuote` 防 `game_over` 重复 `show('win')`；`GameView` 拆分语录 effect（修复结算卡顿 `effect_update_depth_exceeded`） |
| 2026-06-06 | **离席 UX**：离开改 SPA 回大厅（`intentionalLeave` 防误重连）；`PartnerLeftOverlay` 替代「对手已离开」error toast；对局中离席展示比分；房主离席后客人自动回主菜单 |
| 2026-06-06 | **HMR**：`main.ts` 启动清理 `body > .phase-overlay/.phase-toast` 遗留浮层 |
| 2026-06-06 | **特殊骰开局**：有特殊骰时 `start` → `dice_selection` → 双方选完 → `turn_order`（猜金币）→ `selecting`；无特殊骰仍直接 `turn_order` |
| 2026-06-06 | **首掷背面修复**：`prevRollCount` 初值 `-1` 导致进局误开 `physicsRolling`、骰盘空白；改为仅 `prevRollCount>=0 && rollCount` 递增时开动画，`selecting`+`rollCount===0` 复位 |
| 2026-06-06 | **掷骰标注**：`DiceBoard` 在 `physicsRolling` / CSS `rolling` 期间隐藏活动区 `shortName` |
| 2026-06-06 | **移动端 UX**：确认规则 `startPending` 防闪大厅；对手 `pendingSelection` 显示「对手选分」；选骰 `DicePickSummary`；图鉴手机底部抽屉 |
| 2026-06-06 | **字体与主菜单**：Noto Sans SC + ZCOOL KuaiLe + `OrnamentalDivider`；Google Fonts（国内可能 fallback） |
