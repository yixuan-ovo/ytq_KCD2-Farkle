...
> **最后更新：** 2026-06-05（Git 自动部署 · 断线宽限 · 爆点 toast · 加入/金币定先后）  
...
| `src/components/game/CoinFlipOverlay.svelte` | 开局金币定先后（`turn_order`） |
...
| 类型 | 主要文件 |
|------|----------|
| 阶段提示（爆点 / 换手 / Hot Dice；`game_over` 仍全屏） | `PhaseOverlay.svelte`（爆点/换手：**底部 toast**，`pointer-events: none`，不挡底栏；仅展示 `lastBust` 未保留骰） |
...
- **掷骰动画：** `playDiceRollAnimation`（GSAP，DOM 克隆 overlay）；`GameView` 在己方回合且 `rollCount` 增加时设 `physicsRolling`，`DiceBoard` 测量槽位并播放（约 850ms）；`settings.physicsEnabled` 关闭时回退 CSS `medievalRoll`（游戏内不跟随系统「减少动画」）。
...
| 金币定先后 | `startGame` → `phase: turn_order` + `coinFlip`；双方确认后进入 `dice_selection` |
...
- [x] `PhaseOverlay.svelte`：爆点/换手 toast（不挡底栏）；`game_over` 全屏
...
爆点后 Worker 保持 `selecting` + `lastBust`；toast ~2.2–2.8s 自动 dismiss，**须手动点掷骰**。`turn_end` 仍由下次 `roll` 前 normalize。
