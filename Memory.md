...
> **最后更新：** 2026-06-04（文档同步 · 邀请链接 · 爆点/离开/Hot Dice · 部署说明）  
...
| 路径 | 内容 |
|------|------|
| `src/lib/game/*` | 纯 TS 规则引擎（计分、掷骰、35 种骰子） |
...
| `src/components/layout/TavernAmbience.svelte` | 暗色烛光 + emoji + 悬浮骰 1–6 依次发光 |
...
| `docs/dice/骰子模板.md` | 36 种骰主题 fill/border/pip/icon；生成规则说明 |
...
| `src/lib/assets/diceThemes.json` | 36 种骰主题（`docs/dice/骰子模板.md`） |
...
| URL | 行为 |
|-----|------|
| `/room/{id}` | **推荐邀请链接**（仅房间号）。进大厅 `joinOnly` 表单，客人自填昵称后加入 |
...
- **骰子图鉴：** `DiceCollectionPanel.svelte` 读 `diceRegistry`（35 种），复用 `DiceCard`（`readonly`）+ `DiceWeightLegend`；分类 Tab 与 `CATEGORY_LABELS` 一致；**不**影响对局。
- **背景：** `TavernAmbience.svelte`（烛光 + 暗角 + 低对比 emoji + 骨色骰 SVG 依次高亮）；骰子层在 vignette **之上**；`absolute inset:0`。
...
- **骰盘：** 手机 3×2；**≥768px 六枚一排**；`game-page__stage` 垂直居中（对局无 `spacer`）；**已移除** `ThrowPips`。
- **GameHud：** 左「你」、中「目标」、右「对手」；默认头像 `public/avatars/hud-you.jpg` / `hud-opponent.jpg`；昵称 / 总分放大；当前回合 ♛。
- **骰子贴图：** 对局按 `die.type` 加载 `public/dice/{DieId}/face-hidden.svg`（首掷占位）与 `face-{1..6}.svg` / `face-devil.svg`（`value===0`）；图鉴六面预览仍用 `ivory/` 通用面。主题数据 `diceThemes.json`；生成 `yarn dice:generate` → `scripts/generate-themed-dice.mjs`。
...
- 直达 `/room/:id` 无 `?name=` 时：**留 LobbyView** 预填房间号，避免空昵称 connect
...
- `dice_selection`、`rps`、`draft_rps`（GameView 显示「尚未支持」）
...
| 2026-06-03 | **开局前流程**：lobby 满员「对手已连接」+ 房主「设置规则」；`RulesConfigPanel`；`dice_selection` + `pickDice` WS；`DiceSelector` 三档概率色 |
...
| 2026-06-04 | **TavernAmbience**：悬浮骰 1–6 + 依次发光（JS `litIndex`）；暗角上层；大厅骰子位置微调 |
...
| 2026-06-04 | **docs 整理**：`docs/README.md`；`game/` 权重、`dice/` 主题；删 `骰面/` 草稿 SVG；`.gitignore` 不再忽略 docs |
